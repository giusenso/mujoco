...
// motor-side velocity: back-EMF is generated by the rotor, which backlash decouples from the
// load; without backlash the rotor is rigidly locked to the transmission
static mjtNum dcmotorRotorVelocity(const mjModel* m, const mjData* d, int i) {
  const mjtNum* dynprm = m->actuator_dynprm + mjNDYN*i;
  if (dynprm[9] <= 0) {
    return d->actuator_velocity[m->actuator_outadr[i]];
  }
  mjDCMotorSlots slots = mj_dcmotorSlots(dynprm, m->actuator_gainprm + mjNGAIN*i);
  return d->act[m->actuator_actadr[i] + slots.rotor];
}
...
void mj_fwdActuation(const mjModel* m, mjData* d) {
  ...
  for (int i=0; i < nactuator; i++) {
    ...
    case mjDYN_DCMOTOR: {           // DC motor: up to 5 optional states
      ...
      int adr = act_first;
      mjtNum velocity = dcmotorRotorVelocity(m, d, i);
      ...
      // compute physical voltage to feed into current and temperature equations; the controller
      // measures the load, so it keeps using the transmission velocity
      mjtNum V = dcmotorVoltage(ctrl[uadr], d->actuator_length[oadr],
      ...
      // backlash rotor and deflection: act_dot requires the electromagnetic torque, which is
      // only known after the gain/bias loop; set there, skip the slots here
      if (dynprm[9] > 0) {
        adr += 2;
      }
  ...
  for (int i=0; i < nactuator; i++) {
    ...
    case mjBIAS_DCMOTOR: {          // DC motor: back-EMF only (current-limited)
      ...
      if (te <= 0) {
        mjtNum K = gainprm[1];   // motor constant
        bias -= gain * K * dcmotorRotorVelocity(m, d, i);
      }
      break;
    }
  ...
  for (int i=0; i < nactuator; i++) {
    ...
    // cogging torque: a function of rotor angle, which backlash offsets from the load by the
    // tooth deflection
    mjtNum A = biasprm[0];
    if (A != 0) {
      mjtNum Np = biasprm[1];
      mjtNum phi = biasprm[2];
      mjtNum angle = d->actuator_length[oadr];
      if (dynprm[9] > 0) {
        mjDCMotorSlots slots = mj_dcmotorSlots(dynprm, m->actuator_gainprm + mjNGAIN*i);
        angle += d->act[m->actuator_actadr[i] + slots.deflection];
      }
      force[oadr] += A * mju_sin(Np*angle + phi);
    }
    ...
    // backlash: rotor is no longer rigidly locked to the load, so the load receives the mesh
    // torque instead of the motor torque, and the motor torque accelerates the rotor alone
    mjtNum J_r = dynprm[9];  // rotor inertia (output coordinates)
    if (J_r > 0) {
      ...
      mjDCMotorSlots slots = mj_dcmotorSlots(dynprm, m->actuator_gainprm + mjNGAIN*i);
      int adr = m->actuator_actadr[i];
      mjtNum omega_r = d->act[adr + slots.rotor];
      mjtNum delta = d->act[adr + slots.deflection];
      mjtNum velocity = d->actuator_velocity[oadr];

      // mesh torque, zero while the teeth are inside the deadband (free play)
      mjtNum e = mj_backlashExcursion(delta, b);
      mjtNum torque_mesh = e ? k*e + c*(omega_r - velocity) : 0;

      // rotor accelerates under motor torque minus reaction; deflection tracks the slip rate
      mjtNum torque_motor = force[oadr];
      d->act_dot[adr + slots.rotor] = (torque_motor - torque_mesh) / J_r;
      d->act_dot[adr + slots.deflection] = omega_r - velocity;

      // replace: transmitting both would leak torque across the gap
      force[oadr] = torque_mesh;
    }
  }
